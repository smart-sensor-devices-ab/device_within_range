...
var getDistance = function getDistance(rssi) {
  var n = 2;
  var mp = -69;
  var theDistance = Math.pow(10, (mp - rssi) / (10 * n));
  return theDistance;
};
...
document.getElementById('scan').addEventListener('click', function () {
  ...
  my_dongle.at_central().then(function (data) {
    my_dongle.at_gapscan(4, false).then(function (dev) {
      ...
      withDistance.map(function (j, index) {
      ...
      document.getElementById("numDev").innerHTML = withDistance.length + ' Devices Found.';
...
if ((!parent || !parent.isParcelRequire) && typeof WebSocket !== 'undefined') {
  ...
  var ws = new WebSocket(protocol + '://' + hostname + ':' + "51741" + '/');
